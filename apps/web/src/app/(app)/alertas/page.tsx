...
function AlertRow({ alert, locale }: { alert: AlertItem; locale: string }) {
  ...
  return (
    <div className="flex flex-col gap-1 border-t border-slate-100 py-3 first:border-t-0 sm:flex-row sm:items-start sm:justify-between">
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          {alert.detail && <p className="truncate text-sm text-slate-600">{alert.detail}</p>}
          {alert.residentName && (
      ...
      <time className="shrink-0 text-sm text-slate-400" dateTime={alert.at}>
        {time}
      </time>
    </div>
  );
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-slate-500">Medicación no administrada e incidencias de hoy, por prioridad.</p>
        </div>
